...
- **2026-07-05:** **Režim uzavírání kola: FIXED vs KICKOFF.** `DeadlineOverride` dostal sloupec `mode` (`deadline` je nově nullable). Každé KOLO (STAGE scope) má režim: **FIXED** = celé kolo v jeden termín (ruční `deadline` / automatika), nebo **KICKOFF** = každý zápas kola se uzavře — a u skupin i **odhalí cizí tipy** — svým vlastním výkopem (`match.dateUtc`). Přepínač v `/admin/uzaverky` u jednotky celého kola (op `kickoff`; `set`/`now` resetují na FIXED). `stageDeadline` v KICKOFF = výkop posledního zápasu (celé kolo hotové). **Reveal je teď per-zápas:** `tipy/` i `leaderboard/prehled` už negatují celou stránku jedním deadlinem — každý zápas/kategorie se zobrazí, jakmile je uzamčený (`matchLocked`); pořadí skupin gate `rankingsClosed`, postupy+speciály `specialsClosed`; když není odhalené nic, ukáže se zámek s „první uzávěrkou". `matchLocked`/`matchDeadline` berou `MatchLike {id, stage, dateUtc}` — `saveTips` proto načítá i `dateUtc`. Zvoleno „i skupiny — odhalit per zápas" (fér, bez rizika podvádění; obnovuje předsjednocené chování 6/2026 jako volitelný režim).
